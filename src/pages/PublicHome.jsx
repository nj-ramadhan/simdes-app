    ...
    <section className="public-content">
      ...
      {loading ? <div className="finance-empty">Memuat transparansi desa...</div> : data?.statistik && <>
        ...
        <section className="public-resource-grid">
          <ResourceSummary title="Informasi Lingkungan" items={data.lingkungan} primaryKey="kategori" fields={[{ label: 'Lokasi', key: 'lokasi' }, { label: 'Kondisi', key: 'kondisi' }, { label: 'Tanggal laporan', key: 'tgl_laporan' }]} />
          ...
          <ResourceSummary title="Informasi Aset" items={data.aset} primaryKey="nama_aset" fields={[{ label: 'Kategori', key: 'kategori' }, { label: 'Jumlah', key: 'jumlah' }, { label: 'Kondisi', key: 'kondisi' }, { label: 'Lokasi simpan', key: 'lokasi_simpan' }]} />
        </section>
...
function ResourceSummary({ title, items, primaryKey, fields }) {
  return <article className="public-panel public-resource-panel">
    ...
    {items.length ? <ul className="public-resource-list">{items.map((item, index) => <li className="public-resource-item" key={`${item[primaryKey] || title}-${index}`}>
      <strong>{item[primaryKey] || '-'}</strong>
      <dl>{fields.map(({ label, key }) => <div key={key}><dt>{label}</dt><dd>{item[key] ?? '-'}</dd></div>)}</dl>
    </li>)}</ul> : <p className="public-resource-empty">Belum ada data untuk cakupan wilayah ini.</p>}
  </article>;
}
